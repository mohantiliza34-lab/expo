---
title: Expo Router integration
sidebar_title: Expo Router
description: Track per-route render and interactive timings by enabling the Expo Router integration for Expo Observe.
---

import { Prerequisites, Requirement } from '~/ui/components/Prerequisites';
import { Step } from '~/ui/components/Step';

Expo Observe ships an opt-in integration for [Expo Router](/router/introduction/) that emits per-route `cold_ttr`, `warm_ttr`, and `tti` events tagged with the structural route pattern (for example, `/(tabs)/sessions/[sessionId]`). This lets you compare startup and navigation performance by route in the dashboard instead of looking only at app-wide aggregates.

## Prerequisites

<Prerequisites>
  <Requirement title="Expo SDK 56 or later">
    The Expo Router integration is available on SDK 56 and later. On earlier SDKs, `expo-observe`
    still tracks app-wide metrics, but per-route navigation events are not emitted.
  </Requirement>
  <Requirement title="An app already using Expo Observe">
    Follow [Get started](/eas/observe/get-started/) to install `expo-observe` and create your first
    build.
  </Requirement>
  <Requirement title="`expo-router` installed in the app">
    The integration soft-requires `expo-router`. If the package is not installed, the integration
    becomes a silent no-op.
  </Requirement>
</Prerequisites>

<Step label="1">

## Enable the integration

Call `ExpoObserve.configure()` with the `expo-router` integration flag at module scope, before any screen mounts:

```tsx app/_layout.tsx
import ExpoObserve from 'expo-observe';

ExpoObserve.configure({
  integrations: { 'expo-router': true },
});
```

> **warning** The integration must be enabled before mount and cannot be toggled at runtime. Calling `configure()` after the app has mounted, or toggling the flag mid-session, throws an error.

</Step>

<Step label="2">

## Wrap your root with `ObserveRoot`

`ObserveRoot` includes `AppMetricsRoot` plus the integration context that holds navigation timing state. Replace `AppMetricsRoot.wrap(...)` with `ObserveRoot.wrap(...)`:

```tsx app/_layout.tsx
import ExpoObserve, { ObserveRoot } from 'expo-observe';
import { Stack } from 'expo-router';

ExpoObserve.configure({
  integrations: { 'expo-router': true },
});

function RootLayout() {
  return <Stack />;
}

export default ObserveRoot.wrap(RootLayout);
```

</Step>

<Step label="3">

## Call `useObserve()` in your screens

Use the `useObserve()` hook to get a `markInteractive` that is automatically scoped to the current route. The emitted event is tagged with the screen's route pattern.

```tsx app/(tabs)/index.tsx
import { useObserve } from 'expo-observe';
import { useEffect } from 'react';

export default function Home() {
  const { markInteractive } = useObserve();

  useEffect(() => {
    markInteractive();
  }, [markInteractive]);

  return /* ... */;
}
```

> **info** If the integration is disabled or `expo-router` is not installed, `useObserve()` falls back to the global `AppMetrics.markInteractive`. You can leave the hook in place regardless of integration state.

</Step>

## Metrics

All duration metrics are reported in seconds and tagged with `category: 'navigation'` and `routeName` set to the structural route pattern.

### Per-route time to first render (`cold_ttr`)

**What it measures:** Time from when a navigation action is dispatched - for example link click - to when the destination screen first becomes focused. For the very first focus after app launch, the measurement is taken from when the js bundle is loaded, and the event includes `isAppLaunch: true`.

Emitted at most once per `(route, screenId)` pair within a session.

**Event params:**

| Param         | Type      | Description                                                                |
| ------------- | --------- | -------------------------------------------------------------------------- |
| `routeName`   | `string`  | Structural route pattern, for example `/(tabs)/sessions/[sessionId]`.      |
| `url`         | `string`  | Resolved pathname for the navigation.                                      |
| `routeParams` | `object`  | Resolved route params (for example, `{ sessionId: 'abc' }`).               |
| `isAppLaunch` | `boolean` | `true` when measured against process start, `false` for later navigations. |

### Per-route warm render (`warm_ttr`)

**What it measures:** Same as `cold_ttr`, but for screens that were already rendered before focus — typically because they were preloaded via [`<Link prefetch />`](/router/basics/navigation/#prefetching) or the back navigation happened. 

**Event params:** Same as `cold_ttr`. `isAppLaunch` is always `false`.

### Per-screen time to interactive (`tti`)

**What it measures:** Time from when a navigation action is dispatched to when `markInteractive()` is called on the destination screen. Only the first call per navigation is recorded, so it is safe to call `markInteractive()` multiple times.

**Event params:**

| Param         | Type     | Description                                                          |
| ------------- | -------- | -------------------------------------------------------------------- |
| `routeName`   | `string` | Structural route pattern for the screen.                             |
| `url`         | `string` | Resolved pathname.                                                   |
| `routeParams` | `object` | Resolved route params.                                               |
| `...`         | `any`    | Any custom params passed via `markInteractive({ params: { ... } })`. |

### Notes on `routeName`

`routeName` is a structural pattern, not a resolved URL — `/(tabs)/sessions/[sessionId]`, not `/sessions/abc`. This keeps metrics stable across distinct param values so the dashboard buckets them together. Resolved values are still available on the event via `url` and `routeParams`.

### Preloaded routes

Calls to `router.prefetch()` do not count as a user navigation and never seed a `cold_ttr` or `warm_ttr` measurement. The next user-driven navigation to that route emits `warm_ttr` because the screen has already rendered.

## Notes and troubleshooting

- The integration only activates if `expo-router` is installed at runtime. If it is not installed, `useObserve()` and `ObserveRoot` continue to work but no per-route navigation metrics are emitted.
- The integration must be enabled before mount via `ExpoObserve.configure({ integrations: { 'expo-router': true } })`. Toggling it after the app has mounted throws.
- If `markInteractive()` logs `Calling markInteractive on unmounted screen` or `No metadata available for the current screen`, the call ran outside a screen component or after unmount. Move the call into a `useEffect` inside the screen component.
- For general issues with Expo Observe, see [Troubleshooting](/eas/observe/reference/troubleshooting/).
